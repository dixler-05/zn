...
export default function BarberApp() {
  ...
  useEffect(() => {
    localStorage.setItem("orders", JSON.stringify(الطلبات));
  }, [الطلبات]);
  ...
  const إرسالطلب = () => {
    if (!اسم || !السعر || Number(السعر) <= 0) {
      alert("يرجى إدخال اسم الزبون والسعر بشكل صحيح");
      return;
    }
  ...
  return (
    <div style={{ maxWidth: 600, margin: "auto", padding: 20, fontFamily: "Arial, sans-serif" }}>
      ...
      <button onClick={() => setLoggedIn(false)} style={{ marginBottom: 20, padding: 8, fontSize: 16 }}>
        تسجيل خروج
      </button>
